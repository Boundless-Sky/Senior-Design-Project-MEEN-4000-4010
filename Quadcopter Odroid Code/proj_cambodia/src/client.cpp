...
float ft = 5; //[ft]
float des_height = ft/3.28; //[meters]
float roll_from_PID;
		...
		// hovering at 1.2 meters (4feet) above ground
            ...
            case 6:
 	    	...
 	    	case 7:
				...
				/*flag roll pitch throttle, yaw.
				flag = 0x1B
				roll/pitch (Hori_Atti_Tilt_Ang) = -30 to 30 degree
				thrust (Vert_Pos) = 0 m to  m
					...
					while(ros::ok()){
						if(T_lost < 0.25){ //tim in sec
							...
							*/
							...
							drone->attitude_control(0x1B, 0, 0, des_height ,yaw_from_PID);   
